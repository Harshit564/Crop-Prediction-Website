...
def profile(request):
    ...
    for reading in readings:
        ph_readings.append(5.1)
        temp_readings.append(0.3)
        moisture_readings.append(0.5)
        humidity_readings.append(0.5)
...
def api_add_readings(request):
    try:
        reading = UserReadings.objects.create(user_id=8,
                                              ph=5.1,
                                              humidity=0.5,
                                              moisture=0.5,
                                              temp=0.3,
                                              datetime=timezone.now())
        new_data = [float(5.2), float(0.5),
                    float(0.5), float(0.3)]
